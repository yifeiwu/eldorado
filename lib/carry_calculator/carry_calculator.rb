class CarryCalculator
  ...
  def add_container(name, capacity)
    return Response.new(false, 'invalid container') unless valid?(name, capacity)
  ...
  def get_manifest(total_weight)
    @lookup_table[total_weight.to_s]
  end
  ...
  def generate_lookup_hash(max_size = 10)
    ...
    weights = @containers.collect(&:capacity)
  ...
  def fill_lookup_table(total_weight, coefs, num_cons)
    if @lookup_table[total_weight.to_s].nil?
      @lookup_table[total_weight.to_s] = coefs.dup
    else
      better_set = num_cons < @lookup_table[total_weight.to_s].inject(:+)
      @lookup_table[total_weight.to_s] = coefs.dup if better_set
    end
  end

  def valid?(name, capacity)
    return false if name.nil? || capacity.nil?
    return false unless capacity.is_a? Integer
